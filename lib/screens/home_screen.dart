...
class _HomeScreenState extends State<HomeScreen>{
  ...
  bool loading=false; String status='Prêt à écouter la radio';

  @override void initState(){super.initState();
    caster=WebViewController()..setJavaScriptMode(JavaScriptMode.unrestricted)..setBackgroundColor(const Color(0xFFF7F7F7))..loadFlutterAsset('assets/caster_player.html');
  ...
  @override Widget build(BuildContext context){final red=const Color(0xFFEF2B2B);return Scaffold(
    appBar:AppBar(title:const Text('RADIO CIWARA',style:TextStyle(fontWeight:FontWeight.w900)),backgroundColor:const Color(0xFF111214),actions:[IconButton(onPressed:()=>showAboutDialog(context:context,applicationName:'Radio Ciwara 105.5 FM',applicationVersion:'1.0.0',applicationLegalese:'Bamako · Mali'),icon:const Icon(Icons.info_outline))]),
    body:ListView(padding:const EdgeInsets.fromLTRB(16,20,16,32),children:[
      Container(padding:const EdgeInsets.all(24),decoration:BoxDecoration(borderRadius:28,gradient:const LinearGradient(colors:[Color(0xFF17181A),Color(0xFF3A1113)]),border:Border.all(color:Colors.white10)),child:Column(children:[
      ...
      const Text('LECTEUR CASTER.FM',style:TextStyle(fontSize:12,fontWeight:FontWeight.w900,color:Color(0xFFEF2B2B))),
      const SizedBox(height:8),
      Container(height:205,clipBehavior:Clip.antiAlias,decoration:BoxDecoration(color:Colors.white,borderRadius:16),child:WebViewWidget(controller:caster)),
      const SizedBox(height:22),
      _card(Icons.article_outlined,'CIWARA INFO','Le journal écrit de Radio Ciwara',()=>_openUrl(RadioConfig.websiteUrl+'ciwara-info.html')),
      _card(Icons.live_tv_outlined,'CIWARA TV','Regardez la WebTV de Ciwara',()=>_openUrl(RadioConfig.websiteUrl+'ciwara-tv.html')),
      _card(Icons.public,'SITE WEB','Actualités, programmes et direct',()=>_openUrl(RadioConfig.websiteUrl)),
      _card(Icons.radio,'HIT RADIO MAROC','Écouter notre partenaire',()=>_openUrl(RadioConfig.partnerUrl+'live/')),
    ]));}

  Widget _card(IconData icon,String title,String sub,VoidCallback onTap)=>Card(color:Colors.white.withOpacity(.06),child:ListTile(leading:CircleAvatar(backgroundColor:const Color(0xFFEF2B2B),child:Icon(icon,color:Colors.white)),title:Text(title,style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:Text(sub),trailing:const Icon(Icons.chevron_right),onTap:onTap));
  Future<void> _openUrl(String url) async {caster.loadRequest(Uri.parse(url));}
}
